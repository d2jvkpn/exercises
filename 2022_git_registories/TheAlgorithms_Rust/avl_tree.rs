...
impl<T: Ord> AVLTree<T> {
    ...
    fn node_iter(&self) -> NodeIter<T> {
        ...
        let mut node_iter = NodeIter { stack: Vec::with_capacity(cap) };
...
impl<T: Ord> AVLNode<T> {
    pub fn new(value: T) -> Self {
        Self { value, height: 1, left: None, right: None }
    }
...
mod tests {
    ...
    fn is_balanced<T: Ord>(tree: &AVLTree<T>) -> bool {
        tree.node_iter().all(|n| (-1..=1).contains(&n.balance_factor()))
    }
