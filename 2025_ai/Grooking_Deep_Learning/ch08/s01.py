...
relu = lambda x: (x>=0).astype("float") * x   # returns x if x > 0, return 0 otherwise
relu2deriv = lambda x: (x>=0).astype("float") # returns 1 for input > 0, return 0 otherwise
...
test_labels = one_hot_labels(y_test)                                         # shape=(N, 10)
...
for n in range(iterations):
    ...
    if n% 10 == 0 or n == iterations:
        ...
        stdout.write(f"--> I{n:03d}: train_error={train_error:.6f}, train_accuracy={train_acc:.3f}")
        stdout.write(f", test_error={test_error:.6f}, test_accuracy={test_acc:.3f}\n")
