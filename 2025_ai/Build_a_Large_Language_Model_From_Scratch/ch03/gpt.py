...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # assert(d_in  == self.d_out)
        # assert(num_tokens <= context_length)

        # x @ self.W_query: (b, num_tokens, d_in) @ (d_in, d_out)
        ...
        # (b, num_tokens, num_heads, head_dim) * num_heads
        # => (b, num_tokens, d_out)
...
# ReLU: Rectified Linear Unit, ReLU(x)=max(0,x), range[0, ∞)
# SiLU: Sigmoid Linear Unit, SiLU(x)=x⋅σ(x), 1/(1+e**-x)​, range(-∞, ∞)
# GELU: Gaussian Error Linear Unit, GELU(x)=x⋅Φ(x), Φ(x) 是标准正态分布的累积分布函数(CDF), range(-∞, ∞)
# tanh: Hyperbolic Tangent, tanh(x) = (e**x - e**-x)/(e**x + e**-x), range(-1, 1)
